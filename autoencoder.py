...
def run(central_hidden_dim, number_hidden_layers, activation_func, bias, loss, opt,
    ...
    x_train, x_test = util.get_trainset_testset_tensor_from_file(train_file_path, test_file_path,     #carica i due set
                                                                 test, standardize_data)#come torch tensor
    ...
    losses_history = ae_net.train_with_external_crossvalidation(x_train, folds_number, epochs_number, x_test,
    ...
    if encoding == True:
        ...
        util.print_encoding_plot(encoding_history)
    if parameters == True:
